...
@Composable
fun NewsListScreen(viewModel: NewsViewModel,  lifecycleOwner: LifecycleOwner = LocalLifecycleOwner.current) {
    LaunchedEffect(lifecycleOwner.lifecycle.currentState) {
        viewModel.loadNews()
    }
   val newsList: NewsList? by viewModel.newsFlow.collectAsStateWithLifecycle()
